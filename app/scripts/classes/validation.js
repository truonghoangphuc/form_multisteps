import '../../styles/classes/validation.scss';
...
function _showError(el, type) {
  ...
  if (el.type.indexOf('select-') !== -1) {
    _thpSelectWrapper = el.parentElement;
    if (_thpSelectWrapper.classList.contains('thp-select')) {
      _errEl = _thpSelectWrapper.nextElementSibling;
      isCustomSelect = true;
    }
  }
  ...
  if (_needCreateErrorElement) {
    ...
    if (el.type === 'checkbox') {
      ...
      insertAfter(_thpSelectWrapper, _errEl);
    } else {
  ...
  if (isCustomSelect) {
    _thpSelectWrapper.classList.add('invalid');
  }
}
...
function _showSuccess(el) {
  ...
  if (el.type.indexOf('select-') !== -1) {
    ...
    if (_thpSelectWrapper.classList.contains('thp-select')) {
      _errEl = _thpSelectWrapper.nextElementSibling;
      _thpSelectWrapper.classList.remove('invalid');
    }
  }
...
function _validateField(instance, field) {  
  if (field.required || field.getAttribute('pattern')) {
